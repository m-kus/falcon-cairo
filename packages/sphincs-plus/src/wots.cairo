...
use crate::params_128s::{HashOutput, SPX_WOTS_LEN};
...
pub fn wots_pk_from_sig(
    ...
    while let Some(len) = lengths_iter.pop_front() {
        ...
        let chain_pk = chain_hash_128s(ctx, sk.span(), *len, address);
        pk.append(chain_pk);
...
pub fn chain_hash_128s(
    ctx: SpxCtx, input: Span<u32>, length: u32, mut address: Address,
) -> HashOutput {
    let start: u8 = length.try_into().unwrap();
    address.set_hash_address(start);
    let mut output = thash_128s(ctx, address, input);
...
mod tests {
    ...
    fn test_chain_hash() {
        let input = array![0x01020304, 0x05060708, 0x10203040, 0x50607080];
        let output = chain_hash_128s(Default::default(), input.span(), 5, Default::default());
